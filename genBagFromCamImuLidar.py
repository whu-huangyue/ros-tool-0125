...
if __name__ == '__main__':
    ...
    for i in range(len(files)):
        ...
        for j in range(0,len(data),3):
            point_list.append([data[j],data[j+1],data[j+2]])
        # print(data)
        # print(np.size(data))
        # header = header()
        # header.stamp = rospy.rostime.Time.from_sec(ts)
        # header.frame_id = "map"
        point_cloud = PointCloud2()
        point_cloud.header.stamp = rospy.rostime.Time.from_sec(ts)
        point_cloud.header.frame_id = "map"
        # point_cloud.header = header
        point_cloud.height = 1
        point_cloud.width = len(point_list)
        point_cloud.fields = [PointField('x', 0, PointField.FLOAT32, 1),  
                   PointField('y', 4, PointField.FLOAT32, 1),  
                   PointField('z', 8, PointField.FLOAT32, 1)] 
        point_cloud.is_bigendian = False  
        point_cloud.point_step = 12  
        point_cloud.row_step = len(point_list)*12 + 4  
        point_cloud.is_dense = False
        point_cloud.data = np.asarray(point_list, np.float32).tostring()
        # point_cloud.is_dense = False
        # point_cloud.is_bigendian = False
        # point_cloud.data = data.astype(np.float32).flatten()
        ...
        bag_out.write(lidar_topic_name, point_cloud, lidar_ts_ros)
